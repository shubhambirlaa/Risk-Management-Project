...
D_W_M_T_Bills <- read_excel("D:\\BITS\\3RD YEAR\\SEM 2\\ECON\\FRAM\\M-T-BILLS-updated.xlsx")
...
VISAKAIND <- getSymbols.yahoo("VISAKAIND.NS", from = "2020-04-01", to = "2024-03-31", verbose = FALSE, auto.assign = FALSE, periodicity = "monthly")
VISAKAIND <- na.omit(VISAKAIND)
exVISAKAIND <- dailyReturn(VISAKAIND$VISAKAIND.NS.Adjusted)[-1, ] - na.locf(D_W_M_T_Bills_xts$`Monthly Yield %`)
exNSE <- dailyReturn(NSE$NSEI.Adjusted)[-1, ] - na.locf(D_W_M_T_Bills_xts$`Monthly Yield %`)
...
VISHNU <- getSymbols.yahoo("VISHNU.NS", from = "2020-04-01", to = "2024-03-31", verbose = FALSE, auto.assign = FALSE, periodicity = "monthly")
VISHNU <- na.omit(VISHNU)
exVISHNU <- dailyReturn(VISHNU$VISHNU.NS.Adjusted)[-1, ] - na.locf(D_W_M_T_Bills_xts$`Monthly Yield %`)
exNSE <- dailyReturn(NSE$NSEI.Adjusted)[-1, ] - na.locf(D_W_M_T_Bills_xts$`Monthly Yield %`)
