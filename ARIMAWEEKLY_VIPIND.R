VIPIND <- getSymbols.yahoo("VIPIND.NS", from = "2020-04-01", to = "2024-03-31", verbose = F, auto.assign = F, periodicity = "weekly")
VIPIND<- na.omit(VIPIND)
returns_VIPIND <- as.xts(tail(data.frame(VIPIND$VIPIND.NS.Close),-1)/head(data.frame(VIPIND$VIPIND.NS.Close),-1)-1, frequency = 52)
...
plot(VIPIND$VIPIND.NS.Close, main = "VIPIND weekly Closing Prices", xlab = "Date", ylab = "Close Price")
plot(returns_VIPIND, main = "VIPIND Weekly Returns", xlab = "Date", ylab = "Returns")
